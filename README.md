...
- Catálogo individual de archivos FBX, GLB, GLTF y BVH.
...
- Las categorías sin ningún FBX, GLB, GLTF o BVH legible no aparecen hasta que tengan alguno.
...
- Miniaturas reales de piezas y animaciones: la app fotografía cada elemento fuera de pantalla (personajes de frente, objetos del lado más reconocible y, en las animaciones, la pose más expresiva del clip) y guarda un WebP de 256 × 256 en una carpeta `_cache` junto al modelo, con el nombre `<archivo>.webp`. La foto tiene la misma fecha que el modelo: si el modelo cambia, se vuelve a sacar. Las carpetas `_cache` viajan con la carpeta al moverla o copiarla y nunca aparecen como contenido de la biblioteca.
- Al abrir o reescanear se fotografían solo los elementos sin foto, de a una y en segundo plano, con el progreso arriba y botón de pausa; lo que llega por `Cargar Nuevo` y lo visible en la columna derecha pasan primero. Una animación que trae solo el esqueleto se fotografía con el maniquí. Si una foto sale vacía o negra, queda `<archivo>.fallo.txt` con el motivo y no se reintenta hasta que el archivo cambie.
...
- La carga espera a que Windows termine de copiar. Si la categoría ya existe, el contenido se suma adentro; si algún archivo choca con uno existente no se mueve nada y el motivo queda visible. Los archivos sueltos y las carpetas sin FBX, GLB, GLTF ni BVH quedan esperando en `Cargar Nuevo`.
...
- Animaciones sin tirones: los FBX combinan las rotaciones de cada eje por tiempo y, al abrir una animación, los tramos sueltos de hasta 3 cuadros donde un hueso gira de golpe y vuelve se corrigen en memoria, sin modificar el archivo.
...
El plan completo está en el Escritorio como `Plan-Biblioteca-Animaciones-3D-v1.0.0.md`. El estado de las ocho etapas está en `docs/Progreso-Biblioteca-3D-v0.1.2.md`, el uso diario en `docs/Manual-Biblioteca-3D-v0.1.2.md` y el cambio más reciente en `docs/Cambios-Biblioteca-3D-v0.1.9.md`.
